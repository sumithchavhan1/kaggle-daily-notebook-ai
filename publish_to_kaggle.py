...
class KaggleNotebookPublisher:
    ...
    # ==== NEW: adapter matching main.py’s call ====
    def publish_notebook(
        ...
        Args:
            notebook_content: JSON string of nbformat v4 notebook
            dataset_ref: e.g. 'user/dataset-name'
            dataset_title: human-readable title for the notebook
            is_private: whether the kernel should be private
        """
        return self._publish_notebook_internal(
            title=dataset_title,
    ...
    # ==== ORIGINAL logic moved here, signature unchanged internally ====
    def _publish_notebook_internal(
        self,
        title: str,
        content: str,
        dataset_ref: str,
        ...
        try:
            ...
            while version <= max_attempts:
                try:
                    ...
                    username = self.kaggle_api.read_config_file().get("username")
                    metadata = {
